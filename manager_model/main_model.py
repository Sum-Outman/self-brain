...
# Import model registry and data bus
...
from manager_model.emotion_engine import EmotionEngine, get_emotion_engine
...
class ManagementModel(nn.Module):
    ...
    def enhance_with_emotion(self, response, emotion):
        ...
        # Generate emotional modifier based on the emotion analysis
        emotion_modifier = self.emotion_engine.generate_emotional_response(emotion)
        
        # Apply the modifier to the response
        if emotion_modifier:
            enhanced = f"{emotion_modifier} {response}"
    ...
    def get_system_status_summary(self):
        ...
        # Get status from model registry
        registry_status = self.model_registry.get_system_status()
        ...
        summary += f"- Models: {registry_status.get('active_models', 0)}/{registry_status.get('total_models', 0)} active\n"
        summary += f"- Status: {registry_status.get('system_status', 'unknown')}\n"
        summary += f"- My emotional state: {emotion_status.get('primary_emotion', 'neutral')} ({emotion_status.get('intensity', 0)}%)"
