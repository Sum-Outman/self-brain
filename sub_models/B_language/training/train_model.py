...
# -*- coding: utf-8 -*-
"""
Training script for the B_LANGUAGE model
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - B_language_training - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("B_language_training.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SelfBrain.B_language.training")
...
def create_model_architecture(config):
    ...
    logger.info(f"Creating B_LANGUAGE model architecture from scratch")
...
def main():
    """Main training function"""
    logger.info(f"===== B_LANGUAGE Model Training =====")
    ...
    # Load training data
    data_dir = BASE_DIR.parent / "training_data" / "language"
