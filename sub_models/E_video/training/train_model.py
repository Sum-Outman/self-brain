...
# -*- coding: utf-8 -*-
"""
Training script for the E_VIDEO model
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - E_video_training - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("E_video_training.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SelfBrain.E_video.training")
...
def create_model_architecture(config):
    ...
    logger.info(f"Creating E_VIDEO model architecture from scratch")
...
def main():
    """Main training function"""
    logger.info(f"===== E_VIDEO Model Training =====")
    ...
    # Load training data
    data_dir = BASE_DIR.parent / "training_data" / "video"
