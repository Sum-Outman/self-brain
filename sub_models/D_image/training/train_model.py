...
# -*- coding: utf-8 -*-
"""
Training script for the D_IMAGE model
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - D_image_training - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("D_image_training.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SelfBrain.D_image.training")
...
def create_model_architecture(config):
    ...
    logger.info(f"Creating D_IMAGE model architecture from scratch")
...
def main():
    """Main training function"""
    logger.info(f"===== D_IMAGE Model Training =====")
    ...
    # Load training data
    data_dir = BASE_DIR.parent / "training_data" / "image"
