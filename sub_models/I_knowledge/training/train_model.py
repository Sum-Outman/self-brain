...
# -*- coding: utf-8 -*-
"""
Training script for the I_KNOWLEDGE model
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - I_knowledge_training - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("I_knowledge_training.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SelfBrain.I_knowledge.training")
...
def create_model_architecture(config):
    ...
    logger.info(f"Creating I_KNOWLEDGE model architecture from scratch")
...
def main():
    """Main training function"""
    logger.info(f"===== I_KNOWLEDGE Model Training =====")
    ...
    # Load training data
    data_dir = BASE_DIR.parent / "training_data" / "knowledge"
