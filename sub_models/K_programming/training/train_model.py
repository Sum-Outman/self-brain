...
# -*- coding: utf-8 -*-
"""
Training script for the K_PROGRAMMING model
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - K_programming_training - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("K_programming_training.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SelfBrain.K_programming.training")
...
def create_model_architecture(config):
    ...
    logger.info(f"Creating K_PROGRAMMING model architecture from scratch")
...
def main():
    """Main training function"""
    logger.info(f"===== K_PROGRAMMING Model Training =====")
    ...
    # Load training data
    data_dir = BASE_DIR.parent / "training_data" / "programming"
