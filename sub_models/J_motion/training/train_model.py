...
# -*- coding: utf-8 -*-
"""
Training script for the J_MOTION model
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - J_motion_training - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("J_motion_training.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SelfBrain.J_motion.training")
...
def create_model_architecture(config):
    ...
    logger.info(f"Creating J_MOTION model architecture from scratch")
...
def main():
    """Main training function"""
    logger.info(f"===== J_MOTION Model Training =====")
    ...
    # Load training data
    data_dir = BASE_DIR.parent / "training_data" / "motion"
