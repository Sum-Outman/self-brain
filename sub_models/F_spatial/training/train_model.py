...
# -*- coding: utf-8 -*-
"""
Training script for the F_SPATIAL model
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - F_spatial_training - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler("F_spatial_training.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SelfBrain.F_spatial.training")
...
def create_model_architecture(config):
    ...
    logger.info(f"Creating F_SPATIAL model architecture from scratch")
...
def main():
    """Main training function"""
    logger.info(f"===== F_SPATIAL Model Training =====")
    ...
    # Load training data
    data_dir = BASE_DIR.parent / "training_data" / "spatial"
