...
class EnhancedTrainingController:
    ...
    def __init__(self, model_registry, language='zh'):
        ...
        # 初始化进程池（在Windows上使用spawn方法以避免多进程问题）
        self.executor = ProcessPoolExecutor(max_workers=2, mp_context=torch.multiprocessing.get_context('spawn'))
    ...
    def start_individual_training(self, model_name: str, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 在后台启动训练
            future = self.executor.submit(self._train_model, training_id, model_name, training_config)
            self.training_status["active_trainings"][training_id]["future"] = future
    ...
    def start_joint_training(self, model_names: List[str], config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 在后台启动训练
            future = self.executor.submit(self._train_joint_models, training_id, model_names, training_config)
            self.training_status["active_trainings"][training_id]["future"] = future
    ...
    def _train_joint_models(self, training_id: str, model_names: List[str], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 准备联合训练数据
            dataset = self._prepare_joint_training_data(model_names)
            ...
            for epoch in range(epochs):
                ...
                for batch_idx, (multi_inputs, multi_targets) in enumerate(train_loader):
                    # 数据增强处理
                    augmented_inputs = {}
                    augmented_targets = {}
                    for model_name in model_names:
                        if model_name in multi_inputs and model_name in multi_targets:
                            aug_input, aug_target = self._apply_data_augmentation(
                                multi_inputs[model_name], 
                                multi_targets[model_name], 
                                model_name
                            )
                ...
                with torch.no_grad():
                    for multi_inputs, multi_targets in val_loader:
                        batch_loss = 0.0
                        for model_name, model in models.items():
                            if model_name in multi_inputs and model_name in multi_targets:
                                outputs = model(multi_inputs[model_name])
                                loss = criteria[model_name](outputs, multi_targets[model_name])
                                weight = model_weights.get(model_name, 1.0)
    ...
    def _prepare_joint_training_data(self, model_names: List[str]) -> Optional[Dataset]:
        ...
        # 检查联合数据集是否有数据
        if not self._has_training_data(joint_dataset):
            self.logger.warning(f"联合训练数据集没有足够的训练数据 | Joint training dataset doesn't have enough training data")
            # 可以选择创建额外的示例数据或返回空
