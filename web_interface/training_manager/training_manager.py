...
class TrainingManager:
    ...
    def _train_model(self, model_id: str, model: Any, config: Dict[str, Any], callback: Callable = None):
        """Internal method to train a model in a separate thread"""
        try:
            ...
            patience = config.get('patience', 5)  # For early stopping
            ...
            for epoch in range(epochs):
                ...
                self.training_state[model_id]['progress'] = progress
                ...
                for batch_idx in range(num_batches):
                    ...
                    # In a real implementation, this would call model.train() or similar
                    # For now, we'll just simulate training
                    batch_loss = random.random() * 0.1  # Random loss between 0 and 0.1
                    ...
                    for metric in ['accuracy', 'precision', 'recall']:
                        ...
                        epoch_metrics[metric] += random.random() * 0.3 + 0.7  # Random value between 0.7 and 1.0
                ...
                if val_data and len(val_data) > 0:
                    # Simulate validation
                    val_loss = random.random() * 0.1  # Random loss between 0 and 0.1
                    for metric in ['accuracy', 'precision', 'recall']:
                        val_metrics[metric] = random.random() * 0.3 + 0.6  # Random value between 0.6 and 0.9
                
                # Update metrics in training state
                current_metrics = {
                    'loss': epoch_loss,
                    'val_loss': val_loss,
                ...
                self.training_state[model_id]['metrics'] = current_metrics
                
                # Log the epoch
                self.training_logger.log_epoch(model_id, epoch + 1, current_metrics)
                ...
                if val_loss < best_val_loss:
                    ...
                    # Save checkpoint
                    checkpoint_path = self.checkpoint_manager.save_checkpoint(model_id, model, current_metrics, epoch + 1, is_best=True)
                    self.training_state[model_id]['best_checkpoint'] = checkpoint_path
                else:
                    patience_counter += 1
                    if patience_counter >= patience:
                        logger.info(f"Early stopping triggered for model {model_id} after {epoch + 1} epochs")
                        break
                
                # Log epoch completion
                logger.info(f"Model {model_id} - Epoch {epoch + 1}/{epochs} completed - Loss: {epoch_loss:.4f} - Val Loss: {val_loss:.4f}")
            
            # After training is complete, load the best checkpoint
            if 'best_checkpoint' in self.training_state[model_id]:
                model = self.checkpoint_manager.load_checkpoint(model_id, self.training_state[model_id]['best_checkpoint'])
                self.training_state[model_id]['model'] = model
